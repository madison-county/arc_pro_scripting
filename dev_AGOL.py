...
def main():
    ...
    roads_routes_transfer('Roads', 'Are you connected to the server? [Y or N]:\n')
    roads_routes_transfer('Routes', 'Transfer routes to the G Drive? [Y or N]:\n')
...
def downloadFromAGOL(backupFolder, initialStartTime, ARCGIS_PASSWORD):    # Downloads the data from AGOL to the backup folder location; calls the extraction process.
    ...
    print('Temp file in AGOL deleted \t\t\t\t\t\t Line: {} \t{}'.format(get_line(), time.ctime()))
    ...
    print('Variables created \t\t\t\t\t\t\t Line: {} \t{}'.format(get_line(), time.ctime()))
    ...
    outFile.close()
    print('Backup Summary Completed \t\t\t\t\t\t Line: {} \t{}'.format(get_line(), time.ctime()))
